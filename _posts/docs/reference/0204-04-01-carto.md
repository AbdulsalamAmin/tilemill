...
This TileMill build uses the Mapnik 4 Node bindings. CartoCSS stylesheets are
compiled into Mapnik XML with the Mapnik 4-compatible Carto reference before
TileMill renders preview tiles, exports, or the Mapnik XML file.

## Symbolizers

[Mapnik](http://mapnik.org/), the rendering software TileMill is built upon, provides a number
of fundamental style and symbolizer types out of which to build complex styles.
Each type has its own set of configurable properties.

The CartoCSS reference bundled with TileMill exposes the Mapnik 4 style and
symbolizer groups below:

1. Map (global map properties)
2. Polygon (for polygons)
3. Line (for lines & polygons)
4. Markers (for points, lines, & polygons)
5. Shield (for points & lines)
6. Line Pattern (for lines & polygons)
7. Polygon Pattern (for polygons)
8. Raster (for rasters)
9. Point (for points)
10. Text (for points, lines, and polygons)
11. Building
12. Debug
13. Dot
